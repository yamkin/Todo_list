...
function List({ items, isRemoveble, addList, onClick, active, onRemove }) {
  const removeList = (id) => {
    onRemove(id);
  };
    ...
    <ul onClick={onClick}>
      {items.map((item) => (
        <li
          ...
          } p-2.5 grid grid-cols-[max-content_1fr_max-content] items-center mx-auto max-w-[200px] rounded cursor-pointer ${
            !addList
              ? 'hover:bg-white ease-in duration-200 hover:shadow-[0_2px_10px_rgba(0,0,0,0.05)]'
              : null
          }`}>
          <i>
            {item.icon ? (
              ...
              <i className={`mr-[13px] flex rounded-full w-2.5 h-2.5 bg-[${item.color}]`}></i>
            )}
          </i>
          <div>{item.name}</div>
          <i onClick={() => removeList(item.id)} className="group">
            {isRemoveble ? (
              <svg
                width="10"
                height="10"
